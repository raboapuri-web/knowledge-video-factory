...
if(data.chapterId!=='chapter4'||data.status!=='chapter_preview_ready'||data.shots.length!==70||
 data.sourceParagraphCount!==74||data.fps!==30||data.durationFrames<1000||
 !['prologue','chapter1','chapter2','chapter3'].every(id=>approved.approvedChapters.some(c=>
   c.chapterId===id&&c.status==='user_approved_final_do_not_rerender')))
 throw Error('Previous three locked chapters or chapter-four original review manifest unavailable');
...
if(previous!==total||segments.length!==6||segments.some((s,i)=>s.frames<=0||
 ...
 throw Error('Chapter 4 six-way parallel segment plan is not continuous');
const result={chapterId:'chapter4',version:1,sourceApprovedOriginal:'src/approved_original_narration.md',
 fps:30,totalFrames:total,totalScenes:70,plannedDiagrams:27,
 renderWidth:1280,renderHeight:720,
 segments,segmentCount:segments.length};
fs.mkdirSync(path.join(root,'out'),{recursive:true});
fs.writeFileSync(path.join(root,'out/chapter4-segment-plan.json'),JSON.stringify(result,null,2)+'\n');
