...
const edits=JSON.parse(fs.readFileSync(path.join(episode,'src/chapter-edits/chapter2.json'),'utf8'));
if(edits.chapterId!=='chapter2'||edits.shots.length!==52)
 throw Error('Chapter 2 custom asset staging is bound to its 52 reviewed scenes');
...
for(const id of backgroundIDs){
 ...
 if(!fs.existsSync(to)||fs.statSync(to).size!==fs.statSync(from).size)
  throw Error('Chapter 2 asset not staged '+id);
 copied++;
}
if(copied<5)throw Error('Too few additional chapter-two backgrounds staged');
console.log('Validated and staged '+copied+' explicitly storyboarded chapter-two PNG backgrounds, including research/lodging/city custom scenes.');
