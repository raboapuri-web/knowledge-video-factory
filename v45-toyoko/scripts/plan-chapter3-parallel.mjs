...
if(data.chapterId!=='chapter3'||data.status!=='chapter_preview_ready'||data.shots.length!==61||
 data.sourceParagraphCount!==66||data.fps!==30||data.durationFrames<1000||
 !['prologue','chapter1','chapter2'].every(id=>approved.approvedChapters.some(c=>
   c.chapterId===id&&c.status==='user_approved_final_do_not_rerender')))
 throw Error('First two locked chapters or chapter-three original review manifest unavailable');
...
if(previous!==total||segments.length!==6||segments.some((s,i)=>s.frames<=0||
 ...
 throw Error('Chapter 3 six-way parallel segment plan is not continuous');
const result={chapterId:'chapter3',version:1,sourceApprovedOriginal:'src/approved_original_narration.md',
 fps:30,totalFrames:total,totalScenes:61,plannedDiagrams:22,
 renderWidth:1280,renderHeight:720,
 segments,segmentCount:segments.length};
fs.mkdirSync(path.join(root,'out'),{recursive:true});
fs.writeFileSync(path.join(root,'out/chapter3-segment-plan.json'),JSON.stringify(result,null,2)+'\n');
