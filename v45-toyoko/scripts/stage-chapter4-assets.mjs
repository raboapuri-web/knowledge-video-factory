...
const edits=JSON.parse(fs.readFileSync(path.join(episode,'src/chapter-edits/chapter4.json'),'utf8'));
if(edits.chapterId!=='chapter4'||edits.shots.length!==70)
 throw Error('Chapter 4 custom asset staging is bound to its 70 reviewed scenes');
...
for(const id of backgroundIDs){
 ...
 if(!fs.existsSync(to)||fs.statSync(to).size!==fs.statSync(from).size)
  throw Error('Chapter 4 asset not staged '+id);
 copied++;
}
if(copied<5)throw Error('Too few additional chapter-four backgrounds staged');
console.log('Validated and staged '+copied+' explicitly storyboarded chapter-four PNG backgrounds, including research/lodging/city custom scenes.');
