...
const plan=JSON.parse(fs.readFileSync(path.join(root,'out/chapter3-segment-plan.json'),'utf8'));
...
if(plan.chapterId!=='chapter3'||data.chapterId!=='chapter3'||data.shots.length!==61||
 plan.totalFrames!==data.durationFrames||plan.segmentCount!==6||plan.segments.length!==6||
 !['prologue','chapter1','chapter2'].every(id=>approvals.approvedChapters.some(c=>
...
const files=plan.segments.map(s=>{
 ...
 expected+=s.frames;
 const file=path.join(root,'out/chapter3-parts','chapter3-part-'+s.id,
   'chapter3-part-'+s.id+'.mp4');
...
if(expected!==data.durationFrames)throw Error('Missing original chapter-three video frames');
const concat=path.join(root,'out/chapter3-concat.txt');
fs.writeFileSync(concat,files.map(s=>"file '"+s.file.replaceAll("'","'\\''")+"'").join('\n')+'\n');
const picture=path.join(root,'out/chapter3-picture.mp4');
...
const target=path.join(root,'out/toyoko-chapter3-feedback-v1.mp4');
...
if(!verified.streams.some(s=>s.codec_type==='audio'&&s.codec_name==='aac')||
 ...
 throw Error('Original voice missing from finished chapter-three MP4');
const report={verified:true,chapterId:'chapter3',userReviewRequired:true,
 ...
 scenes:61,uniqueDiagramDesigns:22,parallelWorkers:6,
...
fs.writeFileSync(path.join(root,'out/chapter3-review-verification.json'),JSON.stringify(report,null,2)+'\n');
console.log('VERIFIED chapter3, '+data.durationFrames+' exact frames, 6 parallel parts, 52 different scene recommendations, 14 unique diagrams, approved original VOICEVOX with subtitles.');
