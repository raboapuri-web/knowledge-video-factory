...
const edits=JSON.parse(fs.readFileSync(path.join(episode,'src/chapter-edits/epilogue.json'),'utf8'));
if(edits.chapterId!=='epilogue'||edits.shots.length!==22)
 throw Error('Epilogue custom asset staging is bound to its 70 reviewed scenes');
...
for(const id of backgroundIDs){
 const a=bg.get(id);
 if(!a)throw Error('Unregistered epilogue background '+id);
 ...
 if(!fs.existsSync(to)||fs.statSync(to).size!==fs.statSync(from).size)
  throw Error('Epilogue asset not staged '+id);
 copied++;
}
if(copied<4)throw Error('Too few additional epilogue backgrounds staged');
console.log('Validated and staged '+copied+' explicitly storyboarded epilogue PNG backgrounds, including research/lodging/city custom scenes.');
