...
const C={bg:'#030607',ocean:'#0a3f68',ocean2:'#061d33',land:'#496a46',land2:'#78905c',ice:'#d9e3df',cloud:'#e7efed',gold:'#d4ad63',green:'#2f8a59',red:'#b64743',purple:'#8b55a6',paper:'#eee8dc',blue:'#7aa5bf'};

export const KNOWN_RICH_VISUALS=[
...
const Stars=({p}:{p:number})=><AbsoluteFill style={{background:'radial-gradient(ellipse at 50% 35%,#0a1320 0%,#020507 58%,#010202 100%)'}}>
  {Array.from({length:92}).map((_,i)=>{const x=(i*137)%1920,y=(i*83)%980,s=1+(i%4)*.7,o=.22+(i%5)*.09;return <div key={i} style={{position:'absolute',left:x,top:y,width:s,height:s,borderRadius:'50%',background:'#edf5ff',opacity:o*(.75+.25*Math.sin(p*8+i)),boxShadow:i%11===0?'0 0 10px rgba(185,220,255,.8)':'none'}}/>})}
</AbsoluteFill>;

const Continents=({shift}:{shift:number})=><svg viewBox="0 0 1200 600" style={{position:'absolute',left:-90+shift,top:95,width:'145%',height:'78%',opacity:.98,filter:'drop-shadow(0 8px 12px rgba(0,0,0,.25))'}}>
  <g fill={C.land} stroke={C.land2} strokeWidth="4">
    <path d="M70 175 L120 118 L190 92 L260 105 L315 142 L302 185 L252 196 L228 238 L181 258 L145 226 L104 230 L76 205 Z"/>
    <path d="M245 260 L286 283 L316 337 L302 402 L270 470 L235 505 L218 448 L226 386 L208 330 Z"/>
    <path d="M420 132 L470 115 L520 124 L542 150 L514 168 L468 160 Z"/>
    <path d="M472 190 L533 177 L581 202 L594 258 L575 315 L546 382 L507 355 L482 300 L452 248 Z"/>
    <path d="M545 128 L616 92 L700 96 L748 116 L812 108 L878 135 L950 128 L1012 153 L1066 190 L1048 222 L990 218 L962 248 L906 242 L856 271 L806 250 L758 285 L710 260 L678 220 L620 218 L590 178 Z"/>
    <path d="M955 326 L1002 310 L1056 334 L1082 370 L1056 401 L1005 394 L972 364 Z"/>
    <path d="M336 70 L390 52 L423 72 L408 103 L362 106 Z"/>
  </g>
  <g fill={C.ice} opacity=".9"><path d="M0 38 Q300 -5 600 26 T1200 35 L1200 0 L0 0Z"/><path d="M250 540 Q600 515 950 542 L980 600 L220 600Z"/></g>
</svg>;

const EarthGlobe=({p,x=960,y=500,size=760,controlled=false,united=false,night=false}:{p:number;x?:number;y?:number;size?:number;controlled?:boolean;united?:boolean;night?:boolean})=>{
  const spin=lerp(p,0,-58);
  return <div style={{position:'absolute',left:x,top:y,width:size,height:size,transform:`translate(-50%,-50%) rotate(${lerp(p,-2,3)}deg)`,borderRadius:'50%',boxShadow:'0 0 0 3px rgba(120,196,230,.24),0 0 38px rgba(78,170,220,.45),0 0 110px rgba(30,110,160,.26),inset -105px -55px 150px rgba(0,0,0,.84)',background:`radial-gradient(circle at 32% 24%,#1872a4 0%,${C.ocean} 42%,${C.ocean2} 78%,#02070c 100%)`,overflow:'hidden'}}>
    <Continents shift={spin}/>
    <div style={{position:'absolute',inset:-30,opacity:.47,transform:`translateX(${lerp(p,-8,16)}px) rotate(${lerp(p,-2,2)}deg)`,background:'radial-gradient(ellipse at 26% 30%,rgba(255,255,255,.72) 0 2%,transparent 7%),radial-gradient(ellipse at 64% 22%,rgba(255,255,255,.55) 0 2%,transparent 8%),radial-gradient(ellipse at 70% 58%,rgba(255,255,255,.55) 0 3%,transparent 9%),radial-gradient(ellipse at 34% 66%,rgba(255,255,255,.4) 0 3%,transparent 10%)',filter:'blur(8px)'}}/>
    {Array.from({length:15}).map((_,i)=><div key={i} style={{position:'absolute',left:60+(i*109)%650,top:80+(i*71)%560,width:120+(i%4)*45,height:15+(i%3)*8,borderRadius:'50%',background:'rgba(239,246,244,.22)',filter:'blur(9px)',transform:`rotate(${(i%5)*13-24}deg) translateX(${lerp(p,-10,20)}px)`}}/>)}
    <div style={{position:'absolute',inset:0,background:night?'linear-gradient(95deg,transparent 20%,rgba(0,0,0,.08) 42%,rgba(0,0,0,.88) 79%,#000 100%)':'linear-gradient(95deg,rgba(255,255,255,.05),transparent 46%,rgba(0,0,0,.68) 88%)'}}/>
    {night&&Array.from({length:34}).map((_,i)=><div key={'city'+i} style={{position:'absolute',left:330+(i*53)%390,top:130+(i*37)%430,width:2+(i%3),height:2+(i%3),borderRadius:'50%',background:'#f6c86c',opacity:.35+(i%5)*.1,boxShadow:'0 0 6px #f4bd58'}}/>)}
    {(controlled||united)&&<svg viewBox={`0 0 ${size} ${size}`} style={{position:'absolute',inset:0,width:'100%',height:'100%'}}>
      {Array.from({length:7}).map((_,i)=><path key={i} d={`M ${size*.5} ${size*.5} Q ${size*(.08+(i%4)*.24)} ${size*(.08+(i%3)*.34)} ${size*(.17+(i*137%64)/100)} ${size*(.18+(i*73%62)/100)}`} fill="none" stroke={controlled?'rgba(190,67,78,.8)':'rgba(74,185,112,.8)'} strokeWidth={3+(i%2)} strokeDasharray="10 14" strokeDashoffset={-lerp(p,0,80)}/>) }
    </svg>}
    <div style={{position:'absolute',inset:0,borderRadius:'50%',boxShadow:'inset 70px 20px 80px rgba(255,255,255,.05),inset -80px -25px 120px rgba(0,0,0,.64)'}}/>
  </div>;
};

const DoomBust=({p,x=1490,y=475,s=1}:{p:number;x?:number;y?:number;s?:number})=><div style={{position:'absolute',left:x,top:y,width:360*s,height:470*s,transform:`translate(-50%,-50%) scale(${1+Math.sin(p*Math.PI)*.025})`}}>
  <div style={{position:'absolute',left:'1%',top:'0%',width:'98%',height:'70%',background:'radial-gradient(ellipse at 50% 34%,rgba(42,116,75,.75),rgba(5,26,16,.94) 62%,transparent 63%)',clipPath:'polygon(50% 0,91% 19%,100% 80%,77% 68%,50% 100%,23% 68%,0 80%,9% 19%)',filter:'drop-shadow(0 0 42px rgba(45,142,89,.35))'}}/>
  <div style={{position:'absolute',left:'25%',top:'16%',width:'50%',height:'57%',clipPath:'polygon(21% 0,79% 0,95% 20%,86% 80%,66% 100%,34% 100%,14% 80%,5% 20%)',background:'linear-gradient(125deg,#d2d8d4 0%,#86918e 35%,#525c59 72%,#b0b7b4 100%)',boxShadow:'inset -22px -8px 34px rgba(0,0,0,.42),0 0 22px rgba(180,210,190,.14)'}}/>
  <div style={{position:'absolute',left:'34%',top:'34%',width:'32%',height:'7%',background:'#101b17',clipPath:'polygon(0 0,43% 35%,50% 100%,57% 35%,100% 0,93% 80%,58% 72%,50% 100%,42% 72%,7% 80%)'}}/>
  <div style={{position:'absolute',left:'43%',top:'47%',width:'14%',height:'18%',borderLeft:'4px solid #39413f',borderRight:'4px solid #39413f'}}/>
