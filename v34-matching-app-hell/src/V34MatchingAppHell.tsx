...
import {RichSceneV34,type Beat} from './scenes-v34';
...
const splitSubtitle=(text:string)=>{
  ...
  for(const sentence of sentences){
    ...
    for(const part of parts){
      if((buf+part).length>30&&buf){chunks.push(buf);buf=part;}else buf+=part;
    }
...
const Subtitle=({beat,progress,fade}:{beat:Beat;progress:number;fade:number})=>{
  ...
  return <div style={{position:'absolute',left:100,right:100,bottom:42,opacity:fade,display:'flex',justifyContent:'center',pointerEvents:'none'}}><div style={{maxWidth:1600,padding:'15px 32px 17px',borderRadius:15,background:'rgba(3,5,8,.82)',border:'1px solid rgba(244,239,228,.13)',boxShadow:'0 16px 50px rgba(0,0,0,.48)',fontFamily:font,fontWeight:850,fontSize:38,lineHeight:1.42,textAlign:'center',letterSpacing:.25,color:'#f4efe4',textShadow:'0 3px 14px rgba(0,0,0,.92)'}}>{text}</div></div>;
};

export const V34MatchingAppHell:React.FC=()=>{
  ...
  return <AbsoluteFill style={{background:'#020304'}}><AbsoluteFill style={{opacity:fade}}><RichSceneV34 beat={beat}/></AbsoluteFill><Subtitle beat={beat} progress={active.progress} fade={Math.min(1,fade+.24)}/></AbsoluteFill>;
};
