...
const fps=30;
const targetSeconds=125;
const maxSeconds=165;
const segments=[];
let startFrame=0;
...
for(let i=0;i<sync.beats.length;i++){
  ...
  const nextTooLong=!isLast&&(sync.beats[i+1].end-segmentStartSec)>maxSeconds;
  if(isLast||elapsed>=targetSeconds||nextTooLong){
