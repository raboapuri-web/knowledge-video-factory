...
const Subtitle=({beat,progress,fade}:{beat:Beat;progress:number;fade:number})=>{
  ...
  return <div style={{position:'absolute',left:100,right:100,bottom:42,opacity:fade,display:'flex',justifyContent:'center'}}>
    <div style={{maxWidth:1600,padding:'15px 32px 17px',borderRadius:15,background:'rgba(3,5,8,.80)',border:'1px solid rgba(244,239,228,.13)',boxShadow:'0 16px 50px rgba(0,0,0,.46)',fontFamily:font,fontWeight:850,fontSize:38,lineHeight:1.42,textAlign:'center',letterSpacing:.25,color:'#f4efe4',textShadow:'0 3px 14px rgba(0,0,0,.9)'}}>{text}</div>
  </div>;
};

export const V32AcademicComplex:React.FC=()=>{
  ...
  return <AbsoluteFill style={{background:'#030507'}}>
    <AbsoluteFill style={{opacity:fade}}><RichScene beat={beat}/></AbsoluteFill>
    <Subtitle beat={beat} progress={active.progress} fade={Math.min(1,fade+.22)}/>
  </AbsoluteFill>;
};
