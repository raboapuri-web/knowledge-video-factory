...
const GenericMotion=({scene,t,index}:{scene:any;t:number;index:number})=>{
 ...
 if(ch==='chapter4'){
  ...
  if(m==='dependent_child')return <AbsoluteFill><V46_CHILD_RIG_GIRL x={820} y={390} scale={.75} action="idle"/><ParentFatherRig x={280} y={360} scale={.62} action="idle"/><ParentMotherRig x={1320} y={360} scale={.62} action="idle"/>{['heart','house','money'].map((k,i)=><SymbolCard key={k} x={590+i*280} y={760} kind={k} t={t} delay={i*.1}/>)}</AbsoluteFill>;
  ...
  if(m==='village_care')return <AbsoluteFill><PanelImage path="shared/asset-library/背景/BG_V46_CH1_RURAL_HOME.png" x={0} y={0} w={1920} h={1080}/><ParentFatherRig x={280} y={360} scale={.6} action="idle"/><ParentMotherRig x={1260} y={360} scale={.6} action="idle"/><V46_CHILD_RIG_BOY x={820} y={520} scale={.58} action="idle"/></AbsoluteFill>;
