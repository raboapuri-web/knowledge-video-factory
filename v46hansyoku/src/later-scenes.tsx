...
const SceneSpecificMotion=({scene,t,index}:{scene:any;t:number;index:number})=>{
 const sid=String(scene.sceneId||'');
 const cards=(kinds:string[],y=690)=><>{kinds.map((k,i)=><SymbolCard key={k+i} x={170+i*(1580/Math.max(1,kinds.length-1))} y={y+(i%2)*75} kind={k} t={t} delay={i*.07}/>)}</>;
 const oneDoor=(x:number,glow=true)=><AbsoluteFill style={{background:'rgba(4,7,11,.78)'}}><div style={{position:'absolute',left:x,top:190,width:360,height:650,borderRadius:22,border:'10px solid #a79260',background:glow?'radial-gradient(circle at 50% 45%,#d8c68855,#111820 68%)':'#111820',boxShadow:glow?'0 0 70px #d8c68855':'none'}}/><PersonIcon x={x+115+120*ph(t,.1,.8)} y={520} s={.82}/></AbsoluteFill>;
 const humanConditions=({crown=true}:{crown?:boolean}={})=><AbsoluteFill>{crown&&<Crown t={t} x={710} y={90} scale={.88}/>}<PersonIcon x={835} y={420} s={1.1}/>{cards(['money','house','clock','work','school'],760)}</AbsoluteFill>;

 // Chapter 2: repeated modes are deliberately split into different compositions.
 if(sid==='V46-C2-011')return <AbsoluteFill><PersonIcon x={825} y={420} s={1.15}/>{[0,1,2,3,4,5].map((i)=>{const a=i/6*Math.PI*2;return <div key={i} style={{position:'absolute',left:810+Math.cos(a)*650,top:420+Math.sin(a)*310,width:260,height:180,border:'5px solid #8396a6',borderRadius:22,background:'#101722dd'}}><Family x={35} y={24} s={.5}/></div>})}</AbsoluteFill>;
 if(sid==='V46-C2-012')return <AbsoluteFill style={{background:'rgba(3,7,12,.78)'}}><Network t={t} n={14}/><div style={{position:'absolute',left:860,top:430}}><PersonIcon x={0} y={0} s={1.05}/></div></AbsoluteFill>;
 if(sid==='V46-C2-013')return <AbsoluteFill><svg viewBox="0 0 1920 1080" style={{position:'absolute',inset:0}}><path d="M180 760H1740" stroke="#d5c18a" strokeWidth="18"/>{Array.from({length:8},(_,i)=><g key={i}><circle cx={260+i*205} cy={760} r={32+i*3} fill={i<4?'#7f94a4':'#d1ba78'}/><path d={'M'+(260+i*205)+' 720V'+(420-i*25)} stroke="#8093a2" strokeWidth="8" opacity=".55"/></g>)}</svg><PersonIcon x={820} y={330} s={1.15}/></AbsoluteFill>;
 if(sid==='V46-C2-014')return <AbsoluteFill><PanelImage path="shared/asset-library/背景/BG_school.png" x={100} y={110} w={1000} h={830}/><Family x={1270} y={420} s={.9}/><Arrow x1={1250} y1={540} x2={1000} y2={540}/></AbsoluteFill>;
 if(sid==='V46-C2-015')return <AbsoluteFill>{cards(['school','school','heart','clock'],600)}<Family x={210} y={410} s={.85}/><Family x={1380} y={260} s={.68}/><Arrow x1={500} y1={520} x2={1390} y2={390}/></AbsoluteFill>;
 if(sid==='V46-C2-016')return <AbsoluteFill>{Array.from({length:6},(_,i)=><div key={i} style={{position:'absolute',left:180+i*255,top:790-i*105,width:230,height:110,border:'7px solid #c8b577',background:'#26313a'}}><SymbolCard x={55} y={-150} kind={i%2?'house':'money'} t={t} delay={i*.06}/></div>)}</AbsoluteFill>;
 if(sid==='V46-C2-018')return <AbsoluteFill><Family x={210} y={520} s={.72}/>{Array.from({length:5},(_,i)=><div key={i} style={{position:'absolute',left:700+i*205,top:760-i*120,width:160,height:160,borderRadius:24,border:'7px solid #c9b77d',background:'#18212b'}}><SymbolCard x={15} y={12} kind={['school','house','clock','money','work'][i]} t={t} delay={i*.05}/></div>)}</AbsoluteFill>;
 if(sid==='V46-C2-019')return <AbsoluteFill><PersonIcon x={810} y={410} s={1.15}/><div style={{position:'absolute',left:735,top:120,width:450,height:450,borderRadius:'50%',border:'18px solid #cbb77d',borderTopColor:'transparent',transform:'rotate('+(t*160)+'deg)'}}/></AbsoluteFill>;
 if(sid==='V46-C2-020')return <AbsoluteFill><PanelImage path="shared/asset-library/背景/BG_cafenaiso.png" x={220} y={120} w={1480} h={820}/><PersonIcon x={850} y={430} s={1.05}/><SymbolCard x={1330} y={620} kind="heart" t={t}/></AbsoluteFill>;
 if(sid==='V46-C2-025')return oneDoor(780,true);
 if(sid==='V46-C2-026')return <AbsoluteFill><ScaleChoice t={t}/><PersonIcon x={835} y={390} s={1.05}/></AbsoluteFill>;
 if(sid==='V46-C2-027')return <AbsoluteFill><PersonIcon x={835} y={410} s={1.05}/>{[250,460,670,1250,1460,1670].map((x,i)=><Arrow key={x} x1={x} y1={i%2?220:820} x2={930} y2={520} opacity={ph(t,.04+i*.06,.35+i*.06)}/>)}</AbsoluteFill>;
 if(sid==='V46-C2-034')return oneDoor(360,true);
 if(sid==='V46-C2-035')return oneDoor(1200,true);
 if(sid==='V46-C2-036')return <AbsoluteFill><DoorRow t={t}/>{cards(['house','money','clock','work'],820)}</AbsoluteFill>;
 if(sid==='V46-C2-049')return <AbsoluteFill><ChoiceLayers t={t}/><WorldSplit t={t} left="shared/asset-library/背景/BG_TOWN_DAY_WIDE.png" right="shared/asset-library/背景/BG_office.png"/></AbsoluteFill>;
 if(sid==='V46-C2-056')return <AbsoluteFill>{['heart','work','house'].map((k,i)=><div key={k} style={{position:'absolute',left:260+i*540,top:180,width:420,height:690,border:'7px solid #8fa0aa',borderRadius:32,background:'#0d141ddd'}}><SymbolCard x={90} y={65} kind={k} t={t}/>{Array.from({length:7},(_,j)=><div key={j} style={{position:'absolute',left:80+(j%2)*180,top:330+Math.floor(j/2)*80,width:34,height:34,borderRadius:'50%',background:'#d4bf7e',opacity:ph(t,.05+j*.04,.35+j*.04)}}/>)}</div>)}</AbsoluteFill>;

 // Chapter 3: underground biology and human society are never allowed to collapse into the same visual.
 if(sid==='V46-C3-003')return <AbsoluteFill style={{background:'radial-gradient(circle at 42% 50%,#3c281d,#080605 72%)'}}><MoleRat x={300} y={250} s={2.25} t={t}/>{Array.from({length:28},(_,i)=><div key={i} style={{position:'absolute',left:980+((i*83)%760)+80*Math.sin(t*12+i),top:180+((i*137)%720),width:20+(i%4)*7,height:20+(i%3)*6,borderRadius:'50%',background:'#866748',opacity:.75}}/>)}</AbsoluteFill>;
 if(sid==='V46-C3-006')return <AbsoluteFill style={{background:'#160e0a'}}><div style={{position:'absolute',left:180,top:150,width:1560,height:760,borderRadius:360,border:'12px solid #705239',background:'radial-gradient(circle,#39251a,#120c09 72%)'}}>{Array.from({length:5},(_,i)=><MoleRat key={i} x={180+i*250} y={350+(i%2)*120} s={.43} t={t} phase={i*.07}/>) }<MoleRat x={1080} y={270} s={.65} t={t}/></div></AbsoluteFill>;
 if(sid==='V46-C3-008')return <AbsoluteFill style={{background:'#0f0b08'}}><div style={{position:'absolute',left:120,top:160,width:1680,height:720,borderRadius:80,border:'10px solid #7d6042'}}>{Array.from({length:18},(_,i)=><MoleRat key={i} x={80+(i%6)*250} y={70+Math.floor(i/6)*210} s={.34} t={t} phase={i*.03}/>)}</div><div style={{position:'absolute',left:1450,top:230,width:230,height:520,border:'8px solid #c2a36a',borderRadius:120}}/></AbsoluteFill>;
 if(sid==='V46-C3-009')return <AbsoluteFill style={{background:'#0d0907'}}>{Array.from({length:28},(_,i)=><div key={i} style={{position:'absolute',left:100+(i%7)*250,top:120+Math.floor(i/7)*210,opacity:.9}}><MoleRat x={0} y={0} s={.28} t={t} phase={i*.02}/>{i===13&&<div style={{position:'absolute',left:45,top:50,width:70,height:70,borderRadius:'50%',border:'8px solid #d0b671'}}/>}</div>)}</AbsoluteFill>;
 if(sid==='V46-C3-011')return <AbsoluteFill style={{background:'#160f0b'}}><MoleRat x={1350} y={230} s={.9} t={t} queen/>{Array.from({length:4},(_,i)=><MoleRat key={'p'+i} x={1200+i*125} y={610+(i%2)*80} s={.25} t={t}/>)}{Array.from({length:7},(_,i)=><MoleRat key={i} x={120+(i%4)*260} y={170+Math.floor(i/4)*360} s={.38} t={t} phase={i*.06}/>)}</AbsoluteFill>;
 if(sid==='V46-C3-014')return <AbsoluteFill><div style={{position:'absolute',left:80,top:130,width:520,height:760,borderRadius:260,border:'10px solid #a68c5d',background:'#17120e'}}>{Array.from({length:16},(_,i)=><div key={i} style={{position:'absolute',left:80+(i%4)*95,top:110+Math.floor(i/4)*120,width:36,height:18,borderRadius:'50%',background:'#c3a45e'}}/>)}</div><div style={{position:'absolute',left:700,top:130,width:520,height:760,borderRadius:260,border:'10px solid #a68c5d',background:'#17120e'}}>{Array.from({length:13},(_,i)=><div key={i} style={{position:'absolute',left:70+(i%4)*100,top:120+Math.floor(i/4)*130,width:42,height:28,borderRadius:'50%',background:'#d8b84f'}}/>)}</div><div style={{position:'absolute',left:1320,top:130,width:520,height:760,borderRadius:260,border:'10px solid #a68c5d',background:'#17120e'}}><MoleRat x={70} y={250} s={.78} t={t}/></div></AbsoluteFill>;
 if(sid==='V46-C3-017')return <AbsoluteFill style={{background:'#071019'}}><Family x={1230} y={370} s={.9}/><PersonIcon x={300} y={420} s={1.05}/>{Array.from({length:12},(_,i)=>{const q=ph(t,.05+i*.04,.42+i*.04);return <div key={i} style={{position:'absolute',left:520+q*720+(i%3)*25,top:330+(i%4)*105,width:32,height:32,borderRadius:'50%',background:i%2?'#d2bc79':'#8399a9',boxShadow:'0 0 18px #d2bc7988'}}/>})}<Arrow x1={540} y1={530} x2={1240} y2={530}/></AbsoluteFill>;
 if(sid==='V46-C3-024')return <AbsoluteFill><PanelImage path="shared/asset-library/背景/BG_kenkyu.png" x={60} y={90} w={870} h={880}/><div style={{position:'absolute',left:1050,top:160,width:680,height:700,borderRadius:330,border:'10px solid #876846',background:'#170f0b'}}><MoleRat x={180} y={210} s={.9} t={t} queen/></div><Arrow x1={1420} y1={520} x2={850} y2={520}/></AbsoluteFill>;
 if(sid==='V46-C3-025')return <AbsoluteFill style={{background:'#120c09'}}>{[260,650,1040,1430].map((x,i)=><MoleRat key={x} x={x} y={360+80*Math.sin(i)} s={.68} t={t} phase={i*.1} flip={i%2===1}/>) }<div style={{position:'absolute',left:800,top:390,width:320,height:320,borderRadius:'50%',border:'15px solid #c09d62',boxShadow:'0 0 70px #c09d6244'}}/></AbsoluteFill>;
 if(sid==='V46-C3-026')return <AbsoluteFill style={{background:'#100b08'}}>{Array.from({length:11},(_,i)=><MoleRat key={i} x={100+(i%6)*300} y={180+Math.floor(i/6)*440} s={i===8?.75:.42} t={t} phase={i*.05} queen={i===8}/>)}</AbsoluteFill>;
 if(sid==='V46-C3-028')return <AbsoluteFill><PanelImage path="shared/asset-library/背景/BG_kenkyu.png" x={80} y={120} w={720} h={760}/><div style={{position:'absolute',left:930,top:160,width:820,height:690,border:'8px solid #8697a5',borderRadius:30,background:'#08111bdd'}}>{Array.from({length:7},(_,i)=><div key={i} style={{position:'absolute',left:90+i*95,bottom:90,width:60,height:(90+i*70)*ph(t,.05+i*.05,.45+i*.05),background:i<3?'#7c91a1':'#d0b875',borderRadius:'12px 12px 0 0'}}/>)}<MoleRat x={260} y={160} s={.65} t={t}/></div></AbsoluteFill>;
 if(sid==='V46-C3-029')return <AbsoluteFill><div style={{position:'absolute',left:0,top:0,width:1920,height:500,overflow:'hidden'}}><Tunnel t={t} count={6}/></div><PanelImage path="shared/asset-library/背景/BG_TOWN_DAY_WIDE.png" x={0} y={540} w={1920} h={540}/><Arrow x1={960} y1={430} x2={960} y2={710}/></AbsoluteFill>;
 if(sid==='V46-C3-030')return <AbsoluteFill><PeopleGrid t={t} count={24}/><div style={{position:'absolute',opacity:.18}}><Crown t={1-t} x={710} y={100}/></div></AbsoluteFill>;
 if(sid==='V46-C3-031')return <AbsoluteFill><PanelImage path="shared/asset-library/背景/BG_TOWN_DAY_WIDE.png" x={0} y={0} w={1920} h={1080}/><PeopleGrid t={t} count={24}/></AbsoluteFill>;
 if(sid==='V46-C3-034')return <AbsoluteFill><PanelImage path="shared/asset-library/背景/BG_office.png" x={0} y={0} w={1920} h={1080}/><OfficeWorkerRig x={520} y={390} scale={.68} action="idle" suitColor="#4b5365" pantsColor="#343c45" shirtColor="#eee6d5" hairColor="#2e343b"/><Family x={1320} y={330} s={.8} opacity={.22}/><Arrow x1={760} y1={530} x2={1280} y2={530} opacity={.28}/></AbsoluteFill>;
 if(sid==='V46-C3-036')return <AbsoluteFill><div style={{position:'absolute',left:0,top:0,width:930,height:1080,overflow:'hidden'}}><QueenOrbit t={t}/></div><div style={{position:'absolute',left:990,top:0,width:930,height:1080,overflow:'hidden'}}><PanelImage path="shared/asset-library/背景/BG_office.png" x={990} y={0} w={930} h={1080}/><PeopleGrid t={t} count={12}/></div></AbsoluteFill>;
 if(sid==='V46-C3-037')return <AbsoluteFill><MoleRat x={250} y={390} s={.85} t={t}/><PersonIcon x={1450} y={410} s={1.05}/><Arrow x1={650} y1={520} x2={1330} y2={520}/>{cards(['work','heart','house'],760)}</AbsoluteFill>;
 if(sid==='V46-C3-038')return <AbsoluteFill><PanelImage path="shared/asset-library/背景/BG_office.png" x={80} y={100} w={800} h={830}/><PanelImage path="shared/asset-library/背景/BG_TOWN_DAY_WIDE.png" x={1040} y={100} w={800} h={830}/><div style={{position:'absolute',left:800,top:150,width:320,height:320,borderRadius:'50%',border:'12px solid #7f8e99',opacity:.25}}/></AbsoluteFill>;
 if(sid==='V46-C3-040')return <AbsoluteFill><DoorRow t={t}/>{cards(['money','house','clock','work'],800)}</AbsoluteFill>;
 if(sid==='V46-C3-043')return <AbsoluteFill><PanelImage path="shared/asset-library/背景/BG_TOWN_NIGHT_WIDE.png" x={0} y={0} w={1920} h={1080}/><Crown t={t} x={710} y={100}/>{cards(['money','house','clock','work'],780)}</AbsoluteFill>;
 if(sid==='V46-C3-044')return <AbsoluteFill><PeopleGrid t={t} count={24}/><div style={{position:'absolute',opacity:.2+.25*(1-t)}}><Crown t={1-t} x={710} y={80}/></div></AbsoluteFill>;
 if(sid==='V46-C3-045')return <AbsoluteFill><PersonIcon x={820} y={410} s={1.1}/><div style={{position:'absolute',left:250,top:250,width:360,height:520,border:'8px solid #82919c',borderRadius:28,opacity:.45}}/><div style={{position:'absolute',left:1310,top:250,width:360,height:520,border:'8px solid #82919c',borderRadius:28,opacity:.45}}/></AbsoluteFill>;
 if(sid==='V46-C3-047')return <AbsoluteFill><PersonIcon x={220} y={420} s={1}/>{['house','money','clock','work','heart'].map((k,i)=><React.Fragment key={k}><SymbolCard x={530+i*250} y={250+(i%2)*300} kind={k} t={t} delay={i*.06}/>{i<4?<Arrow x1={690+i*250} y1={360+(i%2)*300} x2={780+i*250} y2={520+((i+1)%2)*200} opacity={.55}/>:null}</React.Fragment>)}</AbsoluteFill>;

 // Chapter 4: consecutive office and queen metaphors get different literal scenes.
 if(sid==='V46-C4-001')return <AbsoluteFill><PanelImage path="shared/asset-library/背景/BG_V46_PEER_COMPANY_OFFICE.png" x={120} y={100} w={1680} h={850}/><PeopleGrid t={t} count={18}/></AbsoluteFill>;
 if(sid==='V46-C4-002')return <AbsoluteFill>{Array.from({length:5},(_,i)=><OfficeWorkerRig key={i} x={170+i*330} y={420+(i%2)*60} scale={.56} action="idle" suitColor="#4b5365" pantsColor="#343c45" shirtColor="#eee6d5" hairColor="#2e343b"/>)}<svg viewBox="0 0 1920 1080" style={{position:'absolute',inset:0}}><path d="M120 820H1800" stroke="#d4bf7e" strokeWidth="16" strokeDasharray="30 20" strokeDashoffset={-t*220}/></svg></AbsoluteFill>;
 if(sid==='V46-C4-003')return <AbsoluteFill><OfficeFlow t={t}/><div style={{position:'absolute',left:780,top:160}}><SymbolCard x={0} y={0} kind="money" t={t}/></div></AbsoluteFill>;
 if(sid==='V46-C4-014')return <AbsoluteFill><OfficeWorkerRig x={780} y={390} scale={.7} action="idle" suitColor="#4b5365" pantsColor="#343c45" shirtColor="#eee6d5" hairColor="#2e343b"/><div style={{position:'absolute',left:1280,top:180,opacity:.18}}><Crown t={t} x={0} y={0} scale={.55}/></div></AbsoluteFill>;
 if(sid==='V46-C4-015')return <AbsoluteFill><Network t={t} n={18}/><OfficeWorkerRig x={810} y={400} scale={.68} action="idle" suitColor="#4b5365" pantsColor="#343c45" shirtColor="#eee6d5" hairColor="#2e343b"/>{cards(['money','work','house'],790)}</AbsoluteFill>;
 if(sid==='V46-C4-016')return <AbsoluteFill><PeopleGrid t={t} count={28}/><Family x={1390} y={390} s={.66} opacity={.45}/><Arrow x1={650} y1={540} x2={1390} y2={540}/></AbsoluteFill>;
 if(sid==='V46-C4-052')return <AbsoluteFill style={{background:'#120c09'}}><QueenOrbit t={t}/></AbsoluteFill>;
 if(sid==='V46-C4-053')return <AbsoluteFill><PanelImage path="shared/asset-library/背景/BG_TOWN_NIGHT_WIDE.png" x={0} y={0} w={1920} h={1080}/><PeopleGrid t={t} count={26}/><div style={{position:'absolute',left:760,top:110,width:400,height:260,border:'8px solid #83939f',borderRadius:40,opacity:.18}}/></AbsoluteFill>;

 // Epilogue: every adjacent conceptual sentence receives a different frame grammar.
 if(sid==='V46-E-006')return <AbsoluteFill><PanelImage path="shared/asset-library/背景/BG_oneroom_night.png" x={180} y={80} w={1560} h={900}/><Actor kind="hero" x={780} y={430} scale={.72}/></AbsoluteFill>;
 if(sid==='V46-E-008')return <AbsoluteFill><PanelImage path="shared/asset-library/背景/BG_office.png" x={120} y={100} w={1680} h={850}/><Actor kind="hero" x={720} y={430} scale={.74}/>{cards(['work','money'],720)}</AbsoluteFill>;
 if(sid==='V46-E-009')return <AbsoluteFill><Actor kind="hero" x={780} y={420} scale={.74}/><ScaleChoice t={t}/></AbsoluteFill>;
 if(sid==='V46-E-012')return oneDoor(780,true);
 if(sid==='V46-E-013')return <AbsoluteFill><DoorRow t={t}/>{cards(['money','house','clock','work'],820)}</AbsoluteFill>;
 if(sid==='V46-E-017')return <AbsoluteFill><PeopleGrid t={t} count={24}/><div style={{position:'absolute',opacity:.14}}><Crown t={1-t} x={710} y={90}/></div></AbsoluteFill>;
 if(sid==='V46-E-018')return humanConditions({crown:false});
 if(sid==='V46-E-019')return <AbsoluteFill><Actor kind="hero" x={180} y={430} scale={.7}/>{['money','house','clock','work','heart'].map((k,i)=><React.Fragment key={k}><SymbolCard x={520+i*250} y={250+(i%2)*300} kind={k} t={t} delay={i*.07}/>{i<4&&<Arrow x1={680+i*250} y1={360+(i%2)*300} x2={770+i*250} y2={500+((i+1)%2)*210} opacity={.5}/>}</React.Fragment>)}</AbsoluteFill>;
 if(sid==='V46-E-021')return <AbsoluteFill><ScaleChoice t={t}/><Actor kind="hero" x={810} y={350} scale={.7}/></AbsoluteFill>;
 if(sid==='V46-E-024')return <AbsoluteFill><ChoiceLayers t={t}/><Actor kind="hero" x={760} y={420} scale={.72}/></AbsoluteFill>;
 if(sid==='V46-E-031')return <AbsoluteFill><div style={{position:'absolute',left:0,top:0,width:940,height:1080,overflow:'hidden'}}><QueenOrbit t={t}/></div><div style={{position:'absolute',left:980,top:0,width:940,height:1080,overflow:'hidden'}}><PanelImage path="shared/asset-library/背景/BG_TOWN_NIGHT_WIDE.png" x={980} y={0} w={940} h={1080}/><PeopleGrid t={t} count={12}/></div></AbsoluteFill>;

 return null;
};

const GenericMotion=({scene,t,index}:{scene:any;t:number;index:number})=>{
 const specific=SceneSpecificMotion({scene,t,index}); if(specific)return specific;
 const m=String(scene.visualMode||'');
 const ch=String(scene.chapterId||'');
