...
export const Chapter3Scene=({scene,index,frames}:{scene:any;index:number;frames:number})=>{
  ...
  switch(index){
    ...
    case 17:return <AbsoluteFill style={{background:'#0b1016'}}><div style={{position:'absolute',left:260,top:250,width:550,height:500,border:'9px solid #7d8f9d',borderRadius:40}}/><div style={{position:'absolute',right:260,top:250,width:550,height:500,border:'9px solid #c5a66b',borderRadius:40}}/><Mole x={380} y={390} s={.58} t={t}/><BabyMoles x={1280} y={420} count={4}/>{Array.from({length:10},(_,i)=><GeneNode key={i} x={720+520*ease(ph(t,.04+i*.025,.7+i*.015))} y={390+(i%3)*75} r={12+(i%2)*4}/>) }<Arrow x1={800} y1={500} x2={1160} y2={500} o={.35+.6*q}/></AbsoluteFill>;
    ...
    case 32:return <AbsoluteFill><CityBase/><Family x={1040} y={250} s={.72}/><House x={400} y={210} s={1.25}/><V46_CHILD_RIG_BOY x={520+230*q} y={470-45*q} scale={.5}/><Arrow x1={770} y1={500} x2={1050} y2={500} o={.35+.6*q}/><div style={{position:'absolute',left:330,top:680,width:620,height:34,borderRadius:17,background:'#202a32',border:'5px solid #c7b579'}}><div style={{width:(20+75*q)+'%',height:'100%',borderRadius:12,background:'#78917b'}}/></div></AbsoluteFill>;
    ...
    case 37:return <AbsoluteFill><CityBase/><div style={{position:'absolute',left:160,top:180,width:450,height:650,border:'8px solid #87939c',borderRadius:30}}/><div style={{position:'absolute',left:735,top:180,width:450,height:650,border:'8px solid #87939c',borderRadius:30}}/><div style={{position:'absolute',left:1310,top:180,width:450,height:650,border:'8px solid #87939c',borderRadius:30}}/><PersonDot x={330} y={390} s={1.1}/><PersonDot x={905} y={390} s={1.1}/><PersonDot x={1480} y={390} s={1.1}/>{[0,1,2].map(i=><div key={i} style={{position:'absolute',left:205+i*575,top:235,width:360,height:520,border:'14px solid #9d5c58',opacity:.18+.72*q,transform:'rotate('+(i===1?14:-12)+'deg) scale('+(1-.16*q)+')'}}/>)}<div style={{position:'absolute',left:70,right:70,top:90,height:80,background:'#9d5c58',opacity:.15+.55*q}}/></AbsoluteFill>;
    ...
    case 40:return <AbsoluteFill style={{background:'#0a0f15'}}><ColonyBase/><Mole x={1320} y={620} s={.9} t={t} queen/><div style={{position:'absolute',left:1040+180*q,top:160+140*q,width:700-360*q,height:720-280*q,border:'10px solid #9d5c58',borderRadius:340,boxShadow:'0 0 60px #9d5c5855'}}/>{Array.from({length:7},(_,i)=><Mole key={i} x={120+(i%4)*210} y={170+Math.floor(i/4)*300} s={.3} t={t} flip={i%2===1}/>)}</AbsoluteFill>;
    ...
    case 44:return <AbsoluteFill><CityBase/><div style={{opacity:1-q,transform:'scale('+(1-.12*q)+')',transformOrigin:'center'}}><NoCommand/></div><V46_ADULT_MAN_RIG x={720+130*q} y={340} scale={.72} tieVisible={false}/><div style={{opacity:.25+.75*q}}><Family x={1260} y={370} s={.46}/></div><div style={{position:'absolute',left:1120-70*q,top:220-70*q,width:620+140*q,height:620+140*q,borderRadius:'50%',border:'8px dashed #7e8993',opacity:.15+.35*q}}/></AbsoluteFill>;
    ...
    case 46:return <AbsoluteFill style={{background:'#0b1016'}}><ChoiceRoad t={t}/><V46_ADULT_MAN_RIG x={250+220*q} y={340} scale={.68} tieVisible={false}/>{['money','clock','house','work'].map((k,i)=>{const z=ease(ph(t,.04+i*.12,.34+i*.12));return <div key={k} style={{position:'absolute',left:720+i*250-360*z,top:250+i*120+220*z,opacity:1-.5*z,transform:'scale('+(1-.28*z)+')'}}><Card x={0} y={0} w={210} h={120}><div style={{position:'absolute',inset:25,borderRadius:15,background:i===0?'#607a64':i===1?'#5f6e7e':i===2?'#8c765b':'#765f66'}}/></Card></div>})}<div style={{position:'absolute',left:880,top:570,width:160,height:160,borderRadius:'50%',background:'#d6c280',opacity:q,transform:'scale('+(0.5+.7*q)+')'}}/></AbsoluteFill>;
