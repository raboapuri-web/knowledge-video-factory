...
export const getDurationSeconds=()=>Math.max(1,Number(sync.durationSeconds||1));
...
export const getActiveBeatAtSeconds=(seconds:number)=>{
  ...
  const i=Math.max(0,beats.findIndex((b)=>seconds>=b.start&&seconds<b.end));
  const idx=i<0?beats.length-1:i;
  const b=beats[idx];
