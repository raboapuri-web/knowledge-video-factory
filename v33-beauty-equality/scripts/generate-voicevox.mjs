...
for(let i=0;i<script.beats.length;i++){
  ...
  const spoken=beat.speech??beat.narration;
  const q=await fetch(`${base}/audio_query?speaker=${style.id}&text=${encodeURIComponent(spoken)}`,{method:'POST'}).then(r=>r.json());
  q.speedScale=speed;q.pitchScale=-0.02;q.intonationScale=0.86;q.volumeScale=0.96;q.prePhonemeLength=0.09;q.postPhonemeLength=0.11;
  ...
  const raw=path.join(tmp,`beat-${String(i).padStart(3,'0')}.wav`);const padded=path.join(tmp,`beat-${String(i).padStart(3,'0')}-pad.wav`);
