...
コード: shared/remotion-templates/office-worker-rig.tsx
...
import {OfficeWorkerRig} from '../../shared/remotion-templates/office-worker-rig';
...
運用: ナレーションに意味がある場所だけ人物を重ね、字幕は上のレイヤーへ。隣接するbgGroupでは背景を静止させaction/poseのみ切替可能。年齢・衣装・人物像・構図が台本と合わないときは専用人物を新規作成してください。公式visual-template-registry.jsonは管理台帳のversionとの同期が必要なため、今回は共有Reactコンポーネントとして保存し、シーンregistry自体は変更していません。
