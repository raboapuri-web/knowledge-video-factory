...
const Subtitle=({beat,progress}:{beat:Beat;progress:number})=>{
  ...
  return <div style={{position:'absolute',left:110,right:110,bottom:24,display:'flex',justifyContent:'center',opacity,pointerEvents:'none'}}>
    <div style={{maxWidth:1510,padding:'10px 25px 12px',borderRadius:14,background:'rgba(0,0,0,.66)',fontFamily:font,fontWeight:760,fontSize:30,lineHeight:1.4,textAlign:'center',color:'#f7f4ed',textShadow:'0 2px 12px rgba(0,0,0,.98)',boxShadow:'0 8px 30px rgba(0,0,0,.30)'}}>{chunks[k]??chunks[chunks.length-1]}</div>
  </div>;
};
...
const V56=()=>{
  ...
  return <AbsoluteFill style={{background:'#020306',fontFamily:font}}>
    <AbsoluteFill style={{opacity:fade}}><SceneVisual n={a.index+1} progress={a.progress}/></AbsoluteFill>
...
const Root=()=>{
  ...
  return <Composition id='V56AgingEvolution' component={V56} durationInFrames={duration} fps={30} width={1920} height={1080}/>;
};
registerRoot(Root);
