...
const target=path.join(root,'v67-medieval-ransom');
...
const script=fs.readFileSync(path.join(root,'shared/v67/ransom-script.txt'),'utf8').trim();
...
const beats=units.map((u,i)=>{
 ...
 visual:'v67-'+String(i+1).padStart(3,'0')+'-'+u.phase,
...
fs.writeFileSync(path.join(target,'src/script-data.json'),JSON.stringify({videoId:'V67-medieval-ransom',title:'なぜ中世の兵士は、敵を殺すより「生け捕り」にしたがったのか？【百年戦争×捕虜の身代金×戦争の経済史】',beats},null,2));
fs.writeFileSync(path.join(target,'src/scene-data.json'),JSON.stringify(beats,null,2));
fs.writeFileSync(path.join(target,'src/sync-timing.json'),JSON.stringify({durationSeconds:1650,beats:[]}));
for(const n of ['index.tsx','scenes.tsx'])fs.copyFileSync(path.join(root,'shared/v67',n),path.join(target,'src',n));
...
fs.copyFileSync(path.join(root,'shared/v67/SOURCES.md'),path.join(target,'SOURCES.md'));
fs.copyFileSync(path.join(root,'shared/v67/V67_PRODUCTION_SPEC.md'),path.join(target,'V67_PRODUCTION_SPEC.md'));
fs.writeFileSync(path.join(target,'production-manifest.json'),JSON.stringify({productionSystemVersion:3,videoId:'V67-medieval-ransom',title:'Medieval Ransom',sceneMode:'micro-semantic',requiresPreproductionPlan:true,policy:{noGenericFallback:true,noNonconsecutiveBackgroundReuse:true,staticBackgroundWithinContinuityGroup:true,meaningfulForegroundMutation:true,contactSheetRequired:true,sceneCompleteContactSheetRequired:true,measuredVoiceTimingRequired:true,maxConsecutiveSameBackground:2,minBackgroundGroups:140,minScenes:270}},null,2));
console.log('V67: '+beats.length+' scenes / '+group+' backgrounds');