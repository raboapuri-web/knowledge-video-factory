...
const target=path.join(root,'v79-socrates-trial');
...
const beats=units.map((u,i)=>{
 ...
 visual:'v79-'+String(i+1).padStart(3,'0')+'-'+u.phase,
...
for(const n of ['index.tsx','scenes.tsx'])fs.copyFileSync(path.join(root,'shared/v79',n),path.join(target,'src',n));
...
fs.copyFileSync(path.join(root,'shared/v79/SOURCES.md'),path.join(target,'SOURCES.md'));
fs.copyFileSync(path.join(root,'shared/v79/V79_PRODUCTION_SPEC.md'),path.join(target,'V79_PRODUCTION_SPEC.md'));
fs.writeFileSync(path.join(target,'production-manifest.json'),JSON.stringify({productionSystemVersion:3,videoId:'V79-socrates-trial',title:'Socrates Trial',sceneMode:'micro-semantic',requiresPreproductionPlan:true,policy:{noGenericFallback:true,noNonconsecutiveBackgroundReuse:true,staticBackgroundWithinContinuityGroup:true,meaningfulForegroundMutation:true,contactSheetRequired:true,sceneCompleteContactSheetRequired:true,measuredVoiceTimingRequired:true,maxConsecutiveSameBackground:2,minBackgroundGroups:140,minScenes:270}},null,2));
console.log('V79: '+beats.length+' scenes / '+group+' backgrounds');