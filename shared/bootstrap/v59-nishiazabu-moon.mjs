...
const target=path.join(root,'v59-nishiazabu-moon');
...
const script=fs.readFileSync(path.join(root,'shared/v59/nishiazabu-moon-script.txt'),'utf8').trim();
if(!script.startsWith('西麻布のギャラ飲み帰りだった。')||!script.endsWith('明日の朝の牛乳を買った。')) throw new Error('Incorrect V59 script');
...
fs.writeFileSync(path.join(target,'src/script-data.json'),JSON.stringify({videoId:'V59-nishiazabu-moon',title:'西麻布のギャラ飲み帰りに、タクシーの窓から見上げた月',beats},null,2));
...
fs.writeFileSync(path.join(target,'production-manifest.json'),JSON.stringify({
 ...
 videoId:'V59-nishiazabu-moon',title:'西麻布のギャラ飲み帰りに、タクシーの窓から見上げた月'
},null,2));
...
'# V59 西麻布のギャラ飲み帰りに、タクシーの窓から見上げた月\n\n'+
...
console.log('V59 materialized:',beats.length,'shots',sceneStarts.length,'phases',Object.keys(backgrounds).length,'new backgrounds');
