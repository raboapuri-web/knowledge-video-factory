...
const target=path.join(root,'v66-hoplite-frontline');
...
const script=fs.readFileSync(path.join(root,'shared/v66/hoplite-frontline-script.txt'),'utf8').trim();
...
const beats=units.map((u,i)=>{
 ...
 visual:'v66-'+String(i+1).padStart(3,'0')+'-'+u.phase,
...
if(beats.length<140||group<70)throw new Error('Scenes/backgrounds insufficient '+beats.length+'/'+group);
fs.writeFileSync(path.join(target,'script.txt'),script.replace(/\[\[[a-z_]+\]\]\s*/g,'')+'\n');
fs.writeFileSync(path.join(target,'src/script-data.json'),JSON.stringify({videoId:'V66-hoplite-frontline',title:'なぜ古代ギリシャの最前列では、敵に背を向けるほうが危険だったのか？【ファランクス×戦場の心理学】',beats},null,2));
...
for(const n of ['index.tsx','scenes.tsx'])fs.copyFileSync(path.join(root,'shared/v66',n),path.join(target,'src',n));
...
fs.copyFileSync(path.join(root,'shared/v66/SOURCES.md'),path.join(target,'SOURCES.md'));
fs.copyFileSync(path.join(root,'shared/v66/V66_PRODUCTION_SPEC.md'),path.join(target,'V66_PRODUCTION_SPEC.md'));
fs.writeFileSync(path.join(target,'production-manifest.json'),JSON.stringify({productionSystemVersion:3,videoId:'V66-hoplite-frontline',title:'Hoplite Frontline',sceneMode:'micro-semantic',requiresPreproductionPlan:true,policy:{noGenericFallback:true,noNonconsecutiveBackgroundReuse:true,staticBackgroundWithinContinuityGroup:true,meaningfulForegroundMutation:true,contactSheetRequired:true,sceneCompleteContactSheetRequired:true,measuredVoiceTimingRequired:true,maxConsecutiveSameBackground:2,minBackgroundGroups:70,minScenes:140}},null,2));
console.log('V66: '+beats.length+' scenes / '+group+' backgrounds');