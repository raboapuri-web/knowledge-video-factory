...
const target=path.join(root,'v82-basic-income');
...
const script=fs.readFileSync(path.join(root,'shared/v82/basic-income-script.txt'),'utf8').trim();
...
if(beats.length<270||beats.length>360||group<150)throw new Error('Scenes/backgrounds insufficient '+beats.length+'/'+group);
fs.writeFileSync(path.join(target,'script.txt'),script.replace(/\[\[[a-z_0-9]+\]\]\s*/g,'')+'\n');
fs.writeFileSync(path.join(target,'src/script-data.json'),JSON.stringify({videoId:'V82-basic-income',title:'ベーシックインカムは世界を良くするのか？【労働経済学×福祉国家論×行動経済学】',beats},null,2));
fs.writeFileSync(path.join(target,'src/scene-data.json'),JSON.stringify(beats,null,2));
fs.writeFileSync(path.join(target,'src/sync-timing.json'),JSON.stringify({durationSeconds:1550,beats:[]}));
for(const n of ['index.tsx','scenes.tsx'])fs.copyFileSync(path.join(root,'shared/v82',n),path.join(target,'src',n));
...
fs.writeFileSync(path.join(target,'production-manifest.json'),JSON.stringify({productionSystemVersion:3,videoId:'V82-basic-income',title:'Basic Income',sceneMode:'micro-semantic',requiresPreproductionPlan:true,policy:{noGenericFallback:true,noNonconsecutiveBackgroundReuse:true,staticBackgroundWithinContinuityGroup:true,meaningfulForegroundMutation:true,contactSheetRequired:true,sceneCompleteContactSheetRequired:true,measuredVoiceTimingRequired:true,maxConsecutiveSameBackground:2,minBackgroundGroups:150,minScenes:270}},null,2));
console.log('V82: '+beats.length+' scenes / '+group+' backgrounds');