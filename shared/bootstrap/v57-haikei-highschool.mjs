...
const target=path.join(root,'v57-haikei-highschool');
...
const script=fs.readFileSync(path.join(root,'shared/v57/haikei-highschool-script.txt'),'utf8').trim();
if(!script.startsWith('拝啓、高校生の君へ。')) throw new Error('Wrong V57 canonical script');
...
if(beats.length<95||beats.length>230) throw new Error('Unexpected V57 shot count: '+beats.length);

fs.writeFileSync(path.join(target,'src/script-data.json'),JSON.stringify({videoId:'V57-haikei-highschool',title,beats},null,2));
...
fs.writeFileSync(path.join(target,'production-manifest.json'),JSON.stringify({
 ...
 videoId:'V57-haikei-highschool',title
},null,2));
...
fs.writeFileSync(path.join(target,'IMPLEMENTATION_PLAN.md'),`# V57 拝啓、高校生の僕へ
...
`);
console.log(`V57 materialized: ${beats.length} shots / ${new Set(beats.map(b=>b.phase)).size} phases`);
