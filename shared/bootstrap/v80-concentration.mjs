...
const target=path.join(root,'v80-concentration');
...
const script=fs.readFileSync(path.join(root,'shared/v80/concentration-script.txt'),'utf8').trim();
...
const beats=units.map((u,i)=>{
 ...
 visual:'v80-'+String(i+1).padStart(3,'0')+'-'+u.phase,
...
fs.writeFileSync(path.join(target,'src/script-data.json'),JSON.stringify({videoId:'V80-concentration',title:'現代人の集中力は、本当に低下しているのか？【認知心理学×脳科学×情報経済学】',beats},null,2));
fs.writeFileSync(path.join(target,'src/scene-data.json'),JSON.stringify(beats,null,2));
fs.writeFileSync(path.join(target,'src/sync-timing.json'),JSON.stringify({durationSeconds:1350,beats:[]}));
for(const n of ['index.tsx','scenes.tsx'])fs.copyFileSync(path.join(root,'shared/v80',n),path.join(target,'src',n));
...
fs.copyFileSync(path.join(root,'shared/v80/SOURCES.md'),path.join(target,'SOURCES.md'));
fs.copyFileSync(path.join(root,'shared/v80/V80_PRODUCTION_SPEC.md'),path.join(target,'V80_PRODUCTION_SPEC.md'));
fs.writeFileSync(path.join(target,'production-manifest.json'),JSON.stringify({productionSystemVersion:3,videoId:'V80-concentration',title:'Concentration',sceneMode:'micro-semantic',requiresPreproductionPlan:true,policy:{noGenericFallback:true,noNonconsecutiveBackgroundReuse:true,staticBackgroundWithinContinuityGroup:true,meaningfulForegroundMutation:true,contactSheetRequired:true,sceneCompleteContactSheetRequired:true,measuredVoiceTimingRequired:true,maxConsecutiveSameBackground:2,minBackgroundGroups:140,minScenes:270}},null,2));
console.log('V80: '+beats.length+' scenes / '+group+' backgrounds');