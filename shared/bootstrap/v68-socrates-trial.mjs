...
const target=path.join(root,'v68-socrates-trial');
...
const script=fs.readFileSync(path.join(root,'shared/v68/socrates-script.txt'),'utf8').trim();
...
fs.writeFileSync(path.join(target,'src/script-data.json'),JSON.stringify({videoId:'V68-socrates-trial',title:'なぜソクラテスは民主政によって殺されたのか？【古代ギリシャ史×政治哲学×司法制度】',beats},null,2));
...
for(const n of ['index.tsx','scenes.tsx'])fs.copyFileSync(path.join(root,'shared/v68',n),path.join(target,'src',n));
...
fs.copyFileSync(path.join(root,'shared/v68/V68_PRODUCTION_SPEC.md'),path.join(target,'V68_PRODUCTION_SPEC.md'));
fs.writeFileSync(path.join(target,'production-manifest.json'),JSON.stringify({productionSystemVersion:3,videoId:'V68-socrates-trial',title:'Socrates Trial',sceneMode:'micro-semantic',requiresPreproductionPlan:true,policy:{noGenericFallback:true,noNonconsecutiveBackgroundReuse:true,staticBackgroundWithinContinuityGroup:true,meaningfulForegroundMutation:true,contactSheetRequired:true,sceneCompleteContactSheetRequired:true,measuredVoiceTimingRequired:true,maxConsecutiveSameBackground:2,minBackgroundGroups:140,minScenes:270}},null,2));
console.log('V68: '+beats.length+' scenes / '+group+' backgrounds');