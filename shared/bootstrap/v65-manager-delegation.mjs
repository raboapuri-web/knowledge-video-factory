...
const target=path.join(root,'v65-manager-delegation');
...
const script=fs.readFileSync(path.join(root,'shared/v65/delegation-script.txt'),'utf8').trim();
...
fs.writeFileSync(path.join(target,'src/script-data.json'),JSON.stringify({videoId:'V65-manager-delegation',title:'「丸投げ上司」と「任せる上司」は何が違うのか？【権限委譲×組織心理学】',beats},null,2));
...
for(const n of ['index.tsx','scenes.tsx'])fs.copyFileSync(path.join(root,'shared/v65',n),path.join(target,'src',n));
...
fs.copyFileSync(path.join(root,'shared/v65/SOURCES.md'),path.join(target,'SOURCES.md'));
fs.copyFileSync(path.join(root,'shared/v65/V65_PRODUCTION_SPEC.md'),path.join(target,'V65_PRODUCTION_SPEC.md'));
fs.writeFileSync(path.join(target,'production-manifest.json'),JSON.stringify({productionSystemVersion:3,videoId:'V65-manager-delegation',title:'Manager Delegation',sceneMode:'micro-semantic',requiresPreproductionPlan:true,policy:{noGenericFallback:true,noNonconsecutiveBackgroundReuse:true,staticBackgroundWithinContinuityGroup:true,meaningfulForegroundMutation:true,contactSheetRequired:true,sceneCompleteContactSheetRequired:true,measuredVoiceTimingRequired:true,maxConsecutiveSameBackground:2,minBackgroundGroups:65,minScenes:120}},null,2));
console.log('V65: '+beats.length+' scenes / '+group+' backgrounds');