...
const target=path.join(root,'v81-masochism-history');
...
const script=fs.readFileSync(path.join(root,'shared/v81/masochism-script.txt'),'utf8').trim();
...
const beats=units.map((u,i)=>{
 ...
 visual:'v81-'+String(i+1).padStart(3,'0')+'-'+u.phase,
...
fs.writeFileSync(path.join(target,'src/script-data.json'),JSON.stringify({videoId:'V81-masochism-history',title:'なぜマゾヒズムは、人類の進化から消えなかったのか？【性の歴史×神経科学×進化心理学】',beats},null,2));
...
for(const n of ['index.tsx','scenes.tsx'])fs.copyFileSync(path.join(root,'shared/v81',n),path.join(target,'src',n));
...
fs.copyFileSync(path.join(root,'shared/v81/SOURCES.md'),path.join(target,'SOURCES.md'));
fs.copyFileSync(path.join(root,'shared/v81/V81_PRODUCTION_SPEC.md'),path.join(target,'V81_PRODUCTION_SPEC.md'));
fs.writeFileSync(path.join(target,'production-manifest.json'),JSON.stringify({productionSystemVersion:3,videoId:'V81-masochism-history',title:'Masochism History',sceneMode:'micro-semantic',requiresPreproductionPlan:true,policy:{noGenericFallback:true,noNonconsecutiveBackgroundReuse:true,staticBackgroundWithinContinuityGroup:true,meaningfulForegroundMutation:true,contactSheetRequired:true,sceneCompleteContactSheetRequired:true,measuredVoiceTimingRequired:true,maxConsecutiveSameBackground:2,minBackgroundGroups:140,minScenes:270}},null,2));
console.log('V81: '+beats.length+' scenes / '+group+' backgrounds');