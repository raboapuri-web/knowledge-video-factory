...
const target=path.join(root,'v69-free-services');
...
const source=fs.readFileSync(path.join(root,'shared/v69/free-services-script.txt'),'utf8').trim();
...
if(phases.size!==14)throw Error('Expected 14 story phases, got '+phases.size);
const detect=(x)=>{
 const s=x.narration;
...
fs.writeFileSync(path.join(target,'src/script-data.json'),JSON.stringify({videoId:'V69-free-services',title:'なぜ無料サービスが増えるほど、我々は高い代償を払うことになるのか？【行動経済学×注意経済×情報資本主義】',beats},null,2));
...
for(const name of ['index.tsx','scenes.tsx'])fs.copyFileSync(path.join(root,'shared/v69',name),path.join(target,'src',name));
