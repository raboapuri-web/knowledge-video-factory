...
const target=path.join(root,'v60-deadline-paradox');
...
const script=fs.readFileSync(path.join(root,'shared/v60/deadline-script.txt'),'utf8').trim();
...
if(units.length<130)throw new Error('Source too short: '+units.length);
...
if(beats.length<130||group<65)throw new Error('Scenes/backgrounds insufficient '+beats.length+'/'+group);
fs.writeFileSync(path.join(target,'script.txt'),script.replace(/\[\[[a-z_]+\]\]\s*/g,'')+'\n');
fs.writeFileSync(path.join(target,'src/script-data.json'),JSON.stringify({videoId:'V60-deadline-paradox',title:'なぜ締め切り直前にならないとやる気が出ないのか？【時間割引×報酬系】',beats},null,2));
...
fs.writeFileSync(path.join(target,'production-manifest.json'),JSON.stringify({productionSystemVersion:3,videoId:'V60-deadline-paradox',title:'Deadline Paradox',sceneMode:'micro-semantic',requiresPreproductionPlan:true,policy:{noGenericFallback:true,noNonconsecutiveBackgroundReuse:true,staticBackgroundWithinContinuityGroup:true,meaningfulForegroundMutation:true,contactSheetRequired:true,sceneCompleteContactSheetRequired:true,measuredVoiceTimingRequired:true,maxConsecutiveSameBackground:2,minBackgroundGroups:65,minScenes:130}},null,2));
console.log('V60: '+beats.length+' scenes / '+group+' backgrounds');