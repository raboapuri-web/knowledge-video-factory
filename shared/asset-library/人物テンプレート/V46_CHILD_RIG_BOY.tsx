...
export type ChildBoyAction='front'|'run'|'play'|'cry';
export type ChildBoyJoints={
...
export const getChildBoyPose=(
  action:ChildBoyAction,t:number,speed=1.35,overrides:Partial<ChildBoyJoints>={}
):ChildBoyJoints=>{
  const p:ChildBoyJoints={...base};
  ...
  for(const k of Object.keys(overrides) as (keyof ChildBoyJoints)[]){
    const val=overrides[k];if(typeof val==='number'&&Number.isFinite(val))p[k]=val;
  }
  for(const k of Object.keys(p) as (keyof ChildBoyJoints)[])p[k]=clamp(p[k]);
  return p;
};
...
export const V46_CHILD_RIG_BOY=({
  x=780,y=240,scale=1,mirror=false,action='front',walkSpeed=1.35,pose,
  skinColor='#e7b99a',hairColor='#353744',
  topColor='#5a9ec2',pantsColor='#4c6782',
  shoeColor='#364155',ballColor='#e8b34e'
}:ChildBoyProps)=>{
  const frame=useCurrentFrame(),{fps}=useVideoConfig(),t=frame/fps;
  const j=getChildBoyPose(action,t,walkSpeed,pose);
    ...
    <g transform={mirror?'translate(360 0) scale(-1 1)':undefined}>
      <g transform={'translate(0 '+bounce+')'}>
        <g transform={'rotate('+j.bodyLean+' 180 345)'}>
          ...
          {/* Boy's simple T-shirt over everyday pants. */}
          ...
          <path d="M161 206 Q180 219 199 206"
            stroke="#bce4f1" strokeWidth={4}
            fill="none" strokeLinecap="round"/>
          <path d="M146 335 Q180 347 214 335" stroke="#407e9a"
          ...
          <Hinge x={180} y={206} angle={j.headTilt}>
            ...
            <path d="M-46 -97 Q-48 -126 -27 -135 L-26 -146 L-11 -140
              L-1 -153 L15 -143 L30 -149 L31 -136
              Q48 -121 46 -95 Q28 -113 14 -118 Q-6 -101 -27 -107Z"
              fill={hairColor}/>
