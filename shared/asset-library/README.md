...
このフォルダーは **パーツ/** とユーザーが追加した **背景/** を保持します。背景は名称・関連語などのマスター登録対象ですが、既存の動画に自動で全面背景として差し込まれるわけではありません。人物の静止画像共有フォルダーは削除済みですが、再利用可能な関節付きReact人物は **人物テンプレート/** に置いています。動画の文脈・時代・構図に合わない背景は使用せず、動画ごとに新規制作します。共通の「動き・図解のテンプレート」は `shared/remotion-templates/` に引き続き存在し、画像パーツとは別の仕組みです。

## 背景と人物テンプレートを組み合わせる

- 背景の画像：`背景/BG_office.png` など、`catalog.json` の登録済み・承認済み素材。
- 可動人物のコード：`人物テンプレート/office-worker-rig.tsx`（男性会社員。肩・肘・股関節・膝・頭・胴体を制御）。
- 一緒に使う：`人物テンプレート/scene.tsx` の `OfficeWorkerBackgroundScene` と `人物テンプレート/stage-background.mjs`。
