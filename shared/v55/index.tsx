...
const Subtitle=({beat,progress}:{beat:Beat;progress:number})=>{
  ...
  return <div style={{position:'absolute',left:140,right:140,bottom:24,display:'flex',justifyContent:'center',pointerEvents:'none'}}>
    <div style={{maxWidth:1470,padding:'8px 22px 10px',borderRadius:11,background:'rgba(0,0,0,.55)',fontFamily:font,fontWeight:700,fontSize:28,lineHeight:1.36,textAlign:'center',color:'#f7f5ef',textShadow:'0 2px 10px rgba(0,0,0,.96)'}}>{chunks[k]??chunks[chunks.length-1]}</div>
  </div>;
};
...
const V55=()=>{
  ...
  return <AbsoluteFill style={{background:'#020305'}}>
    <AbsoluteFill style={{opacity:fade}}>
      <SceneVisual meta={scene} beatProgress={a.progress} phaseProgress={a.phaseProgress}/>
    </AbsoluteFill>
...
const Root=()=>{
  const d=Math.max(30,Math.ceil(Number(sync.durationSeconds||900)*30));
  return <Composition id='V55PromotionGhost' component={V55} durationInFrames={d} fps={30} width={1920} height={1080}/>;
};
registerRoot(Root);
